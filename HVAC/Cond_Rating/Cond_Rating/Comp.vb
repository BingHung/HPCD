Module Comp
    ...
    Public Sub Comp()
        ...
        Form1.Comp_Discharge_Pressure.Text = C_sat.P.ToString("0.###")
        Pdis = C_sat.P
        ...
        Form1.Comp_Suction_Pressure.Text = E_sat.P.ToString("0.###")
        Psuc = E_sat.P
        ...
        ' ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Calculation
        ' Get masss flow rate
        Form1.Comp_Volumetric_Efficiency.Text = (0.9207 - 0.0756 * (Pdis / Psuc) + 0.0018 * (Pdis / Psuc) ^ 2).ToString("0.###")
        Vcom = CDbl(Form1.Comp_SweptVolume.Text)
        ...
        Ncom = mri * 60 / (CDbl(Form1.Comp_Volumetric_Efficiency.Text) * E_sup.rho * Vcom)


        'Get state2 (HighPressure_Superheated)
        Form1.Comp_Isentropic_Efficiency.Text = (-0.26 + 0.7952 * (Pdis / Psuc) - 0.2803 * (Pdis / Psuc) ^ 2 + 0.0414 * (Pdis / Psuc) ^ 3 - 0.0022 * (Pdis / Psuc) ^ 4).ToString("0.###")
        h1 = Entropy(Influid, "PT", "SI", Psuc, T1)
        h2s = Entropy(Influid, "PS", "SI", Pdis, h1)
        h2 = h1 - (h1 - h2s) / CDbl(Form1.Comp_Isentropic_Efficiency.Text)
        T2 = Temperature(Influid, "PS", "SI", Pdis, h2)

        Form1.Tcond_sup.Text = T2.ToString("0.###")
        ...
        Form1.Wcomp.Text = (mri * (C_sup.i - E_sup.i)).ToString("0.###")
